...
- (NSString *)uploadImage:(UIImage *)image message:(NSString *)message 
{
  ...
  OAMutableURLRequest *theRequest = [[[OAMutableURLRequest alloc] initWithURL:finalURL
                                                                     consumer:[[[TwitterManager sharedTwitterManager] engine] consumer]
                                                                        token:[[[TwitterManager sharedTwitterManager] engine] accessToken] 
                                                                        realm: nil
  ...
  // Create a connection using this request, with the default timeout and caching policy, 
  // and appropriate Twitter request and response types for parsing and error reporting.
  MGTwitterHTTPURLConnection *connection = nil;
  connection = [[MGTwitterHTTPURLConnection alloc] initWithRequest:theRequest 
                                                          delegate:self 
...
- (NSString *)sendRetweet:(NSString *)tweetId {
  
  if (!tweetId){
    return nil;
  }
  
  NSString *path = [NSString stringWithFormat:@"statuses/retweet/%@.%@", tweetId, API_FORMAT];
...
- (NSString *)retweetsForTweet:(NSString *)tweetId count:(int)count {
  
//  if (!tweetId) {
//    return nil;
//  }
//  
//  NSString *path = [NSString stringWithFormat:@"statuses/%@/retweeted_by.json",tweetId];
//  
//  return [self _sendRequestWithMethod:@"GET" path:path queryParameters:nil body:nil requestType:MGTwitterUserInformationRequest responseType:MGTwitterUsers];

  if (!tweetId) {
    return nil;
  }
  
  NSString *task = [[NSString alloc] initWithFormat:@"%@.%@", tweetId, API_FORMAT];
  NSString *path = [NSString stringWithFormat:@"statuses/retweets/%@", task];
  [task release];
...
- (NSString *)repliesForTweet:(NSString *)tweetId count:(int)count {
  ...
  NSString *task = [[NSString alloc] initWithFormat:@"%@.%@", tweetId, API_FORMAT];
  NSString *path = [NSString stringWithFormat:@"related_results/show/%@", task];
  [task release];
...
- (NSString *)friendsFor:(NSString *)username {
  // i.e. follow
  if (!username) {
    return nil;
  }
