...
@implementation FCSecondAlphabetView
...
- (void)drawRect:(CGRect)rect {
  ...
  if (startDrawing) {
    ...
    if (FCCOMMEN.userSelectedCustomDrawingColor) {
      CGContextSetStrokeColorWithColor(UIGraphicsGetCurrentContext(), [FCCOMMEN.rightDrawColor CGColor]);
    }
    else {
    ...
    //UIImageWriteToSavedPhotosAlbum(currentIamge, nil, nil, nil);
    [currentIamge drawInRect:CGRectMake(0, 0, self.frame.size.width, self.frame.size.height)];
...
- (void) touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  UITouch *touch = [touches anyObject];
 // if (controller.firstAlphabetView.isTouched) {
